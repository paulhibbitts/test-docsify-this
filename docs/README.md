...
Enter the URL of a Markdown file into the URL Builder and view that file as a standalone web page in a new Browser tab. The resulting URL can be copied and shared for others to view the same page. The URL Builder automatically transforms the URL of a GitHub Markdown file into the needed raw source URL (i.e. raw.githubusercontent.com).
