      ...
      if (postTitle) {
        keywords.forEach(keyword => {
          ...
          if (!(postContent.includes('iframe') || postContent.includes(':fas') || postContent.includes(':fab') || postContent.includes('embedly-card') || isImage)) {
            ...
            postPageSlug = postUrl.split('/')[1].split('?')[0].replace('0', '');
            console.log(postPageSlug);
            handlePostContent = postPageSlug
            ? escapeHtml(ignoreDiacriticalMarks(postPageSlug))
            : postContent;
          }
          ...
          if (indexTitle >= 0 || indexContent >= 0) {
            ...
            // Initialize matchContent as an empty string
            let matchContent = '';

            // Check if handlePostContent is truthy
            if (handlePostContent) {
              // Extract and process the content with highlighted keywords
              let processedContent = handlePostContent
                .substring(start, end)
                .replace(
                  regEx,
              ...
              // Conditionally add ellipses based on the value of postPageSlug
              if (postPageSlug) {
                // If postPageSlug is not empty, use the processed content directly

                // Function to convert string to title case and replace hyphens with spaces
                function convertToTitleCaseAndReplaceHyphens(str) {
                  return str
                ...
                // Apply the transformation to postPageSlug
                let formattedPostPageSlug = convertToTitleCaseAndReplaceHyphens(postPageSlug);

                matchContent = `<strong>` + formattedPostPageSlug + `</strong>`;
              } else {
                // If postPageSlug is empty, add ellipses before and after the processed content
                matchContent = '...' + processedContent + '...';
              }
            }
